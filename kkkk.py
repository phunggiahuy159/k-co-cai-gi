...
def ILP():
    ...
    Z = solver.IntVar(0, large, 'z')  # Z represents the maximum time when all tasks are completed.

    # Z should be the maximum of (times[i] + d[i]) for all tasks
    for i in range(n):
        solver.Add(Z >= times[i] + d[i])

    # Multi-objective optimization
    solver.Maximize(number_task)  # First, maximize the number of tasks assigned
    status = solver.Solve()
